...
/*	FgEM.x = _____________________________;
	FgEM.y = _____________________________;
	FgEM.z = 0.0;
...
vec3 f_vME(double t, vec3 r, vec3 v)   /* for Moon, force due to Earth */
{
	...
	/* here are the forces in each case */
/*	FgME.x = ______________________________;
	FgME.y = ______________________________;
	FgME.z = 0.0;
*/
...
main(int argc,char *argv[])
{
  ...
  ofstream outfile; 
	...
	if(argc<3){
		fprintf(stderr, "usage: earth_moon [total time][outfile]\n");
		exit(0);
		}
...
if(t>=259200.0){
//vtAold = scalar_vec3sum(-0.050, vtAold);
  
  double r1 = vec3mag(rtAold);
  double r2 = vec3mag(rtMold) -(r1 + 250000.0);
  double hgh = (sqrt((G*Me))/r1);
  double aa = sqrt((2.0*r2)/(r1+r2)) - 1.0;
  double delv1 = hgh*aa;
//   vtAold = scalar_vec3sum(-4.0*delv1, vtAold);
	vtAold.x = -sqrt(G*Me/R0A) - delv1*sin(theta0);
	vtAold.y = 0.0;
	vtAold.z = 0.0;                     // in plane motion only here
	vtAold = vec3sum(vtEold,vtAold);
...
if(t>=590155.2){ 
//if(rtAold.x>=rtMold.x){
  

      double rAMorbit = vec3mag(vec3diff(rtMold,rtAold));
      double vAMorbit = sqrt(G*Mm/(rAMorbit+250000.0));
      vec3 vtA1;
      vec3 sss = scalar_vec3mult((1.0/vec3mag(vtAold)),vtAold);
      vec3 ppp = scalar_vec3mult(vAMorbit,sss);
      vtA1 = vec3sum(vtMold,ppp);
	vtAold = vtA1;
    }
...
*/
	...
	outfile << " " << rtE.x <<  " " << rtE.y <<  " " << vtE.x << " " <<  vtE.y 
			...
			<< " " << rtA.x <<  " " << rtA.y <<  " " << vtA.x << " " << vtA.y << " " << t << endl;
