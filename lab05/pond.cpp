// pond.cpp to check MC integration of unit radius circle area
...
main()
{
   ...
   srand48(seed);                       // seed the number generator 
      ...
      for (int i=0; i<max; i++){
         ...
         z[i] = (drand48()-0.5)*2.;
         r[i] = hypot(x[i],y[i],z[i]);        // distance from origin 
	 hits += r[i]<=1.0 ? 1.0 : 0.0;  // conditional expression
	 ntrials += 1.0;
      }
    double vol = (hits/ntrials)*pow(2.,ntrials);
   cout << "Volume of inscribed sphere= "<< setprecision(9) <<
   		vol <<" , fractional error= "<< fabs((area-PI)/PI) <<endl;
   ...
   for (int i=0; i<max; i++){		// write results into file, hits outside circle      
	...
	cerr << "data stored in pond.dat" << endl;
   outfile.close();
