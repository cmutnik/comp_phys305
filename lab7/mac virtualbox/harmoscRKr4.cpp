...
{
  double xt0,t0,vt0,vt,xt,t,dt,xtold,vtold, dE, E;
...
//	dt = 0.1;
	...
	outfile << t << "\t" << xt0 << "\t" << vt0 << "\t" << xtrue << "\t" << vtrue << "\t" << dE << "\t" << E << endl;
...
/* Here is the loop that propagates the motion:
	...
	for(t=t0; t<Tmax; t+= dt){
		...
		xtold = xt;
		vtold = vt;
		E = (0.5*m*pow(vtold,2.0)) + (0.5*k*pow(xtold,2.0));
		dE = (0.5*m*pow(dv,2.0)) + (0.5*k*pow(dx,2.0));
		outfile << t << "\t" << xt << "\t" << vt << "\t" << xtrue << "\t" << vtrue << "\t" << dE/E << "\t" << E <<endl;
	   }
		double T = Tmax/period;
